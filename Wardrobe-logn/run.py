...
def delete_wardrobe_item(item_id):
    try:
        ...
        item = db.wardrobe.find_one({'_id': item_id, 'userId': user_id})
        ...
        if 'file_path' in item:
            try:
                file_path = item['file_path'].lstrip('/')
                if os.path.exists(file_path):
                    os.remove(file_path)
            except Exception as e:
        ...
        result = db.wardrobe.delete_one({'_id': item_id, 'userId': user_id})
        ...
        print(f"Error deleting wardrobe item: {str(e)}")
        return jsonify({'error': 'Failed to delete item'}), 500
...
def get_wardrobe_items():
    try:
        ...
        wardrobe_items = db.wardrobe.find({'userId': user_id})
        ...
        for item in wardrobe_items:
            ...
            if 'file_path' in item:
                ...
                if not item['file_path'].startswith('/'):
                    item['file_path'] = '/' + item['file_path']
            
            # Convert ObjectId to string for JSON serialization
            item['_id'] = str(item['_id'])
            
            # Map the label to category
            category = category_mapping.get(item['label'], 'accessories')
            categorized_items[category].append(item)
        
        return jsonify(categorized_items)
    except Exception as e:
        app.logger.error(f"Error fetching wardrobe items: {str(e)}")
...
# Import the AvatarClothingGenerator
...
@app.route('/api/check-3d-model/<item_id>', methods=['GET'])
@login_required
def check_3d_model(item_id):
    """Check if a 3D model exists for a wardrobe item"""
    try:
        # Convert string ID to ObjectId
        item_id = ObjectId(item_id)
        ...
        # Check if item has 3D model information
        has_model = item.get('has_3d_model', False)
        model_path = item.get('model_path')
        texture_id = item.get('texture_id')
        ...
        if has_model and model_path:
            ...
            # Verify the texture exists in GridFS
            fs = GridFS(db)
            texture_exists = texture_id and fs.exists(ObjectId(texture_id))
        ...
        print(f"Error checking 3D model: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/generate-3d-model', methods=['POST'])
@login_required
def generate_3d_model():
    """Generate a 3D model from a wardrobe item."""
    try:
        data = request.get_json()
        ...
        # Get the wardrobe item from the database
        item = db.wardrobe.find_one({'_id': ObjectId(item_id)})
        if not item:
            return jsonify({'success': False, 'error': 'Item not found'})

        # Initialize the generator with GridFS support
        generator = AvatarClothingGenerator(
            wardrobe_path='flaskapp/static/image_users',
            output_path='flaskapp/static/avatar_assets',
            db_client=client  # Pass MongoDB client for GridFS
        )

        # Generate the 3D model
        success, texture_id, model_path = generator.generate_top_from_wardrobe(
            item['file_path'],
            str(item['_id'])
        )
        
        if success:
            # Update the wardrobe item with 3D model and texture information
            db.wardrobe.update_one(
...
@app.route('/api/texture/<texture_id>', methods=['GET'])
@login_required
def get_texture(texture_id):
    """Retrieve a texture from GridFS."""
    try:
        # Initialize GridFS
        fs = GridFS(db)
        
        # Try to get the texture from GridFS
        if not fs.exists(ObjectId(texture_id)):
            return jsonify({'error': 'Texture not found'}), 404

        # Get the file from GridFS
        grid_out = fs.get(ObjectId(texture_id))
        ...
        response = send_file(
            ...
            mimetype='image/png',
            as_attachment=False,
            download_name=f'texture_{texture_id}.png'
        )
        
        return response

    except Exception as e:
        print(f"Error retrieving texture: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/generate-3d-clothing', methods=['POST'])
@login_required
def generate_3d_clothing():
    try:
        data = request.get_json()
        if not data or 'wardrobe_item_id' not in data:
            return jsonify({'error': 'Invalid request data'}), 400

        # Get the wardrobe item
        wardrobe_item = db.wardrobe.find_one({
            '_id': ObjectId(data['wardrobe_item_id']),
            'userId': session['user']['_id']
        })
        ...
        # Initialize the clothing generator
        generator = AvatarClothingGenerator(
            wardrobe_path='flaskapp/static/image_users',
            output_path='flaskapp/static/models/generated',
            db_client=mongo
        )

        # Generate 3D model from the wardrobe item
        model_info = generator.generate_from_image(
            image_path=wardrobe_item['file_path'].lstrip('/'),
            category=data['category']
        )

        if not model_info or 'model_url' not in model_info:
        ...
        app.logger.error(f"Error generating 3D clothing: {str(e)}")
        return jsonify({'error': str(e)}), 500
