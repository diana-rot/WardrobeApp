...
def load_dataset():
    ...
    train_mask = np.isin(train_labels, [0, 6])
    test_mask = np.isin(test_labels, [0, 6])

    train_images, train_labels = train_images[train_mask], np.array(train_labels[train_mask] == 6)
    test_images, test_labels = test_images[test_mask], np.array(test_labels[test_mask] == 6)
...
def evaluate_model(dataX, dataY, class_names, n_folds=5):
    ...
    model.save('my_2_class_model.h5')
